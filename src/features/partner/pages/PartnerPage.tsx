  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <Card className="space-y-3">
        <h2 className="text-xl font-bold">Как пользоваться бережно</h2>
      ...
      {dashboard.cycleHistory?.length ? (
        <div className="mt-4 space-y-3">
          {dashboard.cycleHistory.slice(-5).map((cycle) => (
            <div key={cycle.startDate} className="rounded-2xl border border-border p-3 text-sm">
...
function PartnerEmpathyCard({ phase }: { phase?: CyclePhase }) {
  ...
  return (
    <Card className="glass-panel border-primary/20 bg-primarySoft">
